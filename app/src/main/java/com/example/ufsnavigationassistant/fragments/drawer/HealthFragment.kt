...
class HealthFragment : Fragment(R.layout.fragment_health), BuildingAdapter.OnItemClickListener {
    ...
    private fun loadBuildings() {
        ...
        val requestCall: Call<List<Building>> = buildingService.getHealthServices()
        ...
        requestCall.enqueue(object : Callback<List<Building>> {
            override fun onResponse(call: Call<List<Building>>, response: Response<List<Building>>) {
                if(response.isSuccessful) {
                    ...
                    healthBuildingRecycler.adapter = BuildingAdapter(buildingList, this@HealthFragment)
                    healthBuildingRecycler.layoutManager = LinearLayoutManager(context)
                    healthBuildingRecycler.setHasFixedSize(true)

                    //Attach ItemDecorator to draw lines between list items
                    DividerItemDecoration(
                        context, // context
                        (healthBuildingRecycler.layoutManager as LinearLayoutManager).orientation
                    ).apply {
                        // add divider item decoration to recycler view
                        // this will show divider line between items
                        healthBuildingRecycler.addItemDecoration(this)
                    }
